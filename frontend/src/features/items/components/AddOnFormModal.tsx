...
import type { AddOnDetail, ItemMetaIngredient, ItemMetaSauce, ItemRecipeRow, ItemUnitMeta } from "@/types/item";
...
export const AddOnFormModal = ({
  ...
  const sauceSummaries = useMemo(() => {
    return sauceRows.map((row) => {
      ...
      const converted = convertQuantity(quantity, row.unit as ItemMetaIngredient["unit"], sauce.outputUnit, unitMeta);
      if (converted === null) {
        return {
          compatible: false,
          cost: 0,
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      toast.warning("Add-on name is required.");
      return;
    }
  ...
  return (
    <>
      ...
      <ModalContent borderRadius="16px">
        <ModalHeader>{initialData ? "Edit Add-on" : "Create Add-on"}</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <VStack spacing={5} align="stretch">
            <Box>
              ...
              <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                <AppInput label="Add-on Name" value={name} onChange={(event) => setName((event.target as HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement).value)} />
                <AppInput
                  label="Selling Price"
                  type="number"
                  min={0}
              ...
              <FormControl mt={4}>
                ...
                <Textarea
                  ...
                  placeholder="Add a short note for this add-on"
                />
              </FormControl>
              ...
              {initialData ? (
                <HStack mt={4} spacing={3}>
                  ...
                  <Text fontWeight={600}>{isActive ? "Add-on Enabled" : "Add-on Disabled"}</Text>
                </HStack>
              ) : null}
            </Box>
        ...
        <ModalFooter gap={3}>
          ...
          <AppButton onClick={() => void handleSave()} isLoading={loading}>
            {initialData ? "Save Add-on" : "Create Add-on"}
          </AppButton>
        </ModalFooter>
      </ModalContent>
      </Modal>
